...
$router->get('/@{readable_id:.+}', function ($readable_id) {
	$readable_id = strtolower($readable_id);

	//	Nonexistent readable id.
	if (false === $user_id = $GLOBALS['auth']->getID($readable_id)) {
		return false;
	}
...
$router->get('/@{readable_id:.+}/{post_title:.+}-{post_id:\d+}', function ($readable_id, $post_title, $post_id) {
