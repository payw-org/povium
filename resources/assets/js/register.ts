...
if (document.querySelector('#register-main .input-wrapper.readable-id input')) {
	...
	let readableIDInputDOM: HTMLInputElement  = document.querySelector('.input-wrapper.readable-id input')
	let nameInputDOM      : HTMLInputElement  = document.querySelector('.input-wrapper.name input')
			...
			try {
				if (data !== '') {
					...
					if (data['password_return']['err']) {
							...
							passStrengthIndicator.setStrength(data['password_return']['strength'])
					}
				}
			} catch (error) {
				console.log(error)
	...
	if (passwordViewBtn) {
		passwordViewBtn.addEventListener('click', e => { togglePassView() })
	}
