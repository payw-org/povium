window.addEventListener('scroll', function() {
	...
	profileInfo.style.transform = 'translate(0, ' + afterTransform + 'px) scale(' + (offset + 720) / (historyElmDistance + 720) + ')'
...
document.querySelectorAll('.menu-item').forEach(function(menuItem) {
	menuItem.addEventListener('click', function() {
		...
		document.querySelector('.board-container.selected').classList.remove('selected')
		document.querySelector('.board-' + this.getAttribute('data-mi-name')).classList.add('selected')
	})
})